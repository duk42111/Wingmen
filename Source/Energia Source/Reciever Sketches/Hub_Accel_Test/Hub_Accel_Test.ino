...
struct sPacket
{
  ...
  uint8_t message[1];    // Local node message [MAX. 59 bytes]
};
...
void loop()
{
  ...
  if (Radio.receiverOn((unsigned char*)&rxPacket, sizeof(rxPacket), 1000) > 0)
  {
    ...
    Serial.print(" MSG: ");
    if(*rxPacket.message == '1')
    {
      Serial.println("ACCEL TRIGGER, PUT LED ON");
    }
    else
    {
      Serial.println("PUT LED OFF");
    }		
    digitalWrite(RED_LED, LOW);
  }
}
